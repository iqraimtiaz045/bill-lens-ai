// Standalone, self-contained Vercel serverless function.
// This does NOT import from server.ts on purpose — a previous version imported
// '../server' and Vercel's function bundler failed to resolve/include that file
// at runtime (Cannot find module '/var/task/server'). Keeping everything in this
// one file removes that entire class of deployment failure.
// NOTE: server.ts (used for local dev via 'npm run dev') still has its own copy
// of this same logic — if you change the AI prompts/logic, update BOTH files.


import express from 'express';
import { GoogleGenAI, Type } from '@google/genai';

// IMPORTANT: Vercel normally auto-parses the request body itself before it
// reaches this function. That conflicts with Express's own express.json()
// middleware below (used to read the base64 bill image), causing POST
// requests to fail with a 500 error. Disabling Vercel's parser here lets
// Express handle the body like it normally does.
export const config = {
  api: {
    bodyParser: false,
  },
